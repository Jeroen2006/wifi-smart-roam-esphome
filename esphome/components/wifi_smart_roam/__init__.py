...
CONFIG_SCHEMA = cv.Schema({
    ...
    cv.Optional(CONF_BEST_RSSI): sensor.sensor_schema(
        unit_of_measurement="dBm", accuracy_decimals=0, icon="mdi:access-point"
    ),
    cv.Optional(CONF_CURRENT_BSSID): text_sensor.text_sensor_schema(icon="mdi:router-wireless"),
    cv.Optional(CONF_BEST_BSSID):   text_sensor.text_sensor_schema(icon="mdi:router-wireless"),
